...
impl<U: UserRepository> UserService<U> {
    ...
    pub async fn validate_credentials(&self, user_name: String, password: String) -> Result<User, AuthenticationError> {
        ...
        let parsed_hash = PasswordHash::new(user_creds).map_err(|_| AuthenticationError::InvalidCredentials)?;

        Argon2::default().verify_password(password.as_bytes(), &parsed_hash).map_err(|_| AuthenticationError::InvalidCredentials)?;
...
use super::*;
    ...
    async fn test_validate_credentials_with_valid_creds(){
        let repo = MockUserRepository{
            users: [("a@b.com".to_string(), new_user("a@b.com"))].into_iter().collect()
        };
        let svc = UserService::new(repo);

        let result_user = svc.validate_credentials("a@b.com".to_string(), "password".to_string()).await.unwrap();
    ...
    async fn test_fails_to_validate_credentials_invalid_creds(){
        ...
        let result = svc.validate_credentials("a@b.com".to_string(), "wrong password".to_string()).await;
    ...
    async fn test_fails_to_validate_credentials_unknown_user(){
        ...
        let result = svc.validate_credentials("xyx@yue.com".to_string(), "wrong password".to_string()).await;
    ...
    fn new_user(email: &str) -> User {
        User { id: 1, email: email.to_string(), display_name: None, created_at: chrono::Utc::now(), auth: Some(UserAuth {
            id: 1,
            user_id: 1,
